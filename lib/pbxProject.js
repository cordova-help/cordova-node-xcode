...
pbxProject.prototype.addTarget = function(name, type, subfolder, bundleId) {
    ...
    if (targetBundleId) {
        buildConfigurationsList = buildConfigurationsList.map((elem) => {
            ...
            elem.buildSettings.CODE_SIGN_IDENTITY = '$EX_SIGNING_IDENTITY';
            elem.buildSettings.PROVISIONING_PROFILE_SPECIFIER = '$EX_PROVISION';
            elem.buildSettings.DEVELOPMENT_TEAM = '$EX_DEVELOPMENT_TEAM';
            if (elem.name === 'Debug') {
                elem.buildSettings.CODE_SIGN_ENTITLEMENTS = subfolder + '/' + name + '-Debug.entitlements';
            } else {
                elem.buildSettings.CODE_SIGN_ENTITLEMENTS = subfolder + '/' + name + '-Release.entitlements';
            }
            return elem;
        });
    }
